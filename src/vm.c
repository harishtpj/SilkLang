...
static bool invokeFromClass(ObjClass* klass, ObjStr* name,
                            int argCount) {
  Value method;
...
#define READ_CONST() \
    (frame->closure->function->chunk.consts.values[READ_BYTE()])
...
#define BINARY_FUN(valueType, fn) \
    do { \
      if (!IS_NUMBER(peek(0)) || !IS_NUMBER(peek(1))) { \
        runtimeError("Operands must be numbers."); \
        return INTERPRET_RUNTIME_ERROR; \
    ...
    while (true) {
        ...
        case OP_CALL: {
            ...
            if (!callValue(peek(argCount), argCount) || IS_ERR(peek(0))) {
                return INTERPRET_RUNTIME_ERROR;
            }
